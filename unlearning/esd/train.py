...
def load_denoiser(opt, vae_dim, logger, ckpt_name='net_best_fid.tar'):
    ...
    # Ensure only clip is missing (expected behavior in SALAD)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--negative_guidance", type=float, default=1.0, help="ESD Guidance scale (1.0=Neutral, >1.0=Push away)")
    parser.add_argument("--unlearn_lr", type=float, default=1e-5, help="Learning rate for unlearning")
    parser.add_argument("--unlearn_epochs", type=int, default=10, help="Number of epochs to unlearn")
    ...
    denoiser = load_denoiser(opt, vae_opt.latent_dim, logger, ckpt_name='net_best_fid.tar').to(opt.device)
    ...
    try:
        # --- FIX: Pass dummy plot function to prevent crash at end of epoch ---
        trainer.train(
            train_loader, 
            eval_val_loader, 
            eval_val_loader, 
