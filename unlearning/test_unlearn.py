...
def safe_calculate_diversity(activations, times):
    ...
    if num_samples > 0 and safe_times < 1:
        safe_times = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", type=str, required=True, help="The model you want to evaluate"
    )
    ...
    vae_opt_path = pjoin(
        denoiser_opt.checkpoints_dir, denoiser_opt.dataset_name, opt.vae_name, "opt.txt"
    )
    vae_opt = load_options(
        vae_opt_path,
        device,
        {"latent_dim": 256, "activation": "gelu", "n_extra_layers": 1},
    )

    dataset_opt_path = f"checkpoints/{denoiser_opt.dataset_name}/Comp_v6_KLD005/opt.txt"
    evaluator_opt = load_options(
        dataset_opt_path,
        device,
        {
            "max_text_len": 20,
    ...
    ckpt_path = pjoin(
        denoiser_opt.checkpoints_dir,
        denoiser_opt.dataset_name,
        opt.name,
        "model",
        opt.ckpt_name,
    )
    ...
    scheduler = DDIMScheduler(
        num_train_timesteps=denoiser_opt.num_train_timesteps,
        beta_start=denoiser_opt.beta_start,
        beta_end=denoiser_opt.beta_end,
        beta_schedule=denoiser_opt.beta_schedule,
        prediction_type=denoiser_opt.prediction_type,
        clip_sample=False,
    )
    trainer = DenoiserTrainer(denoiser_opt, denoiser_model, vae_model, scheduler)

    temp_opt_path = pjoin(
        os.path.dirname(denoiser_opt_path), "temp_dataset_opt_for_eval.txt"
    )
    try:
        ...
        forget_loader, _ = get_dataset_motion_loader(
            temp_opt_path, 32, opt.forget_test_file.replace(".txt", ""), device=device
        )
        efficacy_save_dir = pjoin(
            denoiser_opt.checkpoints_dir,
            denoiser_opt.dataset_name,
            opt.name,
            "eval_efficacy",
        ...
        trainer.test(
            ...
            forget_loader,
            1,
            save_dir=efficacy_save_dir,
            cal_mm=False,
            save_motion=False,
        )
        console.print(
            f"[green]Efficacy results saved to: {efficacy_save_dir}[/green]\n"
        )

        console.rule("[bold green]📊 Preservation Evaluation (RETAIN set) 📊")
        retain_loader, _ = get_dataset_motion_loader(
            temp_opt_path, 32, opt.retain_test_file.replace(".txt", ""), device=device
        )
        preservation_save_dir = pjoin(
            denoiser_opt.checkpoints_dir,
            denoiser_opt.dataset_name,
            opt.name,
            "eval_preservation",
        ...
        trainer.test(
            ...
            retain_loader,
            1,
            save_dir=preservation_save_dir,
            cal_mm=False,
            save_motion=False,
        )
        console.print(
            f"[green]Preservation results saved to: {preservation_save_dir}[/green]"
        )
