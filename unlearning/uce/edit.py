...
def setup_logging(log_dir, concept_name):
    ...
    log_file = pjoin(log_dir, f"edit_uce_{concept_name}.log")
...
def get_average_embedding_from_captions(_denoiser, captions, batch_size=64):
    ...
    with torch.no_grad(), Progress(
        ...
        for i in range(0, len(captions), batch_size):
            ...
            word_emb, _, _ = _denoiser.clip_model.encode_text(batch)
            word_emb = _denoiser.word_emb(word_emb)
...
def apply_contrastive_uce_kv(denoiser, c_forget, c_retain, logger, lambda_param=1.0):
    ...
    logger.info(
        f"Applying official UCE (Key & Value) with lambda = {lambda_param:.2f}..."
    )

    all_st_transformer_blocks = [
        m
        for m in denoiser.transformer.modules()
        if m.__class__.__name__ == "STTransformerLayer"
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to text file with captions to FORGET.",
    )
    parser.add_argument(
        "--retain_file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Name of the VAE model to use.",
    )
    ...
    opt.vae_name = uce_args.vae_name

    concept_name = (
        os.path.basename(opt.forget_file)
        .replace(".txt", "")
        .replace("train_val-w-", "")
    )
    NEW_EXP_NAME = f"{opt.name}_UCE_{concept_name}"
    ...
    console.rule(
        f"[bold yellow]Official UCE (K/V) Edit: Forgetting '{concept_name}'[/bold yellow]"
    )
    logger.info("Original Model: [cyan]%s[/cyan]", opt.name)
    logger.info("New Experiment: [cyan]%s[/cyan]", NEW_EXP_NAME)
    logger.info("Forget Set File: [red]%s[/red]", opt.forget_file)
    logger.info("Retain Set File: [green]%s[/green]", opt.retain_file)
    logger.info("Lambda (Strength): %.2f", opt.uce_lambda)
    ...
    try:
        ...
        vae_opt_path = pjoin(
            opt.checkpoints_dir, opt.dataset_name, opt.vae_name, "opt.txt"
        )
        vae_opt = get_opt(vae_opt_path, opt.device)
        logger.info("Using VAE model: [cyan]%s[/cyan]", opt.vae_name)

        ckpt_path = pjoin(
            opt.checkpoints_dir, opt.dataset_name, opt.name, "model", "net_best_fid.tar"
        )
        logger.info("Loading Denoiser from: [cyan]%s[/cyan]", ckpt_path)
        ...
        logger.error("Could not find configuration or model file: %s", e)
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            "Loaded %s 'forget' and %s 'retain' captions.",
            len(forget_captions),
            len(retain_captions),
        )

        logger.info("Computing average embedding for FORGET set...")
        forget_embedding = get_average_embedding_from_captions(
            denoiser, forget_captions
        )

        logger.info("Computing average embedding for RETAIN set...")
        retain_embedding = get_average_embedding_from_captions(
            denoiser, retain_captions
        )

        denoiser = apply_contrastive_uce_kv(
            denoiser,
            forget_embedding,
            retain_embedding,
            logger,
            lambda_param=opt.uce_lambda,
        )

        save_path = pjoin(save_dir, "latest.tar")
        logger.info("Saving edited model to: [green]%s[/green]", save_path)
        torch.save(
            {"denoiser": denoiser.state_dict_without_clip(), "epoch": 0}, save_path
        )
        shutil.copy(
            base_opt_path,
            pjoin(opt.checkpoints_dir, opt.dataset_name, NEW_EXP_NAME, "opt.txt"),
        )

        console.rule("[bold green]Official UCE (K/V) Editing Complete!")
        logger.info("To evaluate, use the experiment name: %s", NEW_EXP_NAME)
